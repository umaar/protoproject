...
const alertUserDoesntHaveWebRTCSupport = function (e) {
...
function startVideoAndAR2(video) {
	...
	setInterval(() => {
		...
		canvas.changed = true;
		const markerCount = detector.detectMarkerLite(raster, 190);
...
function startVideoAndAR(video) {
	...
	const detector = new FLARMultiIdMarkerDetector(param, 2);
	detector.setContinueMode(true);
	const frame = 0;
	...
	setInterval(() => {
		...
		const t = new Date();
		const detected = detector.detectMarkerLite(raster, 190);
		for (let idx = 0; idx < detected; idx++) {
			const id = detector.getIdMarkerData(idx);
			var currId;
			if (id.packetLength > 4) {
				currId = -1;
			...
			const zrotation = Math.atan2(cm[0], cm[4]);
			// Console.log("id ", id);
